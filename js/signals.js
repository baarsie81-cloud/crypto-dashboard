...
function timeframeState(candles, interval) {
  ...
  const histogramDirection = signWithTolerance(histogram);
  const macdDirection = signWithTolerance(macdLine - macdSignal);
  const momentumDirection = (rsiDirection + histogramDirection + macdDirection) / 3;
...
export function suggestedLeverage({ score, entry, stop, maxLeverage = 10 }) {
  ...
  return Math.max(1, Math.min(maxLeverage, riskCap, confidenceCap));
}
...
function buildReasons({ bias, states, spreadPct, volumeScore, availability }) {
  const reasons = [];
  if (!availability) reasons.push("Margin niet beschikbaar");
  ...
  if (timeframeBias(states.D) === "NEUTRAAL") reasons.push("1d regime neutraal");
...
export function analyzeMarket({
  ...
  for (const interval of Object.keys(TIMEFRAMES)) {
    ...
    const closed = closedCandles(
      candlesByTimeframe?.[interval] || [],
      TIMEFRAMES[interval].milliseconds,
      now,
    );
  ...
  const availability = instrument.status === "Trading" && instrument.marginTrading !== "none";
  ...
  if (!complete) {
    return {
      ...
      reasons: ["Onvoldoende gesloten candles"],
      spreadPct,
      availability,
      fresh,
      plan: null,
  ...
  const trendDirectional =
    states["60"].trendDirection * 12 +
    states["240"].trendDirection * 16 +
    states.D.trendDirection * 12;
  const momentumDirectional =
    states["60"].momentumDirection * 18 +
    states["240"].momentumDirection * 12;
  ...
  const activityQuality = clamp((Math.log10(Math.max(turnover24h, 1)) - 4) / 2, 0, 1);
  ...
  if (
    availability &&
    fresh &&
    spreadPct <= SIGNAL_LIMITS.maximumSpreadPct &&
    bias !== "NEUTRAAL"
  ) {
    if (
      score >= SIGNAL_LIMITS.actionableScore &&
      higherTimeframeConfirmed &&
      !dailyOpposes &&
      spreadPct <= SIGNAL_LIMITS.actionableSpreadPct
    ) status = bias;
    ...
    fresh &&
    spreadPct <= SIGNAL_LIMITS.maximumSpreadPct &&
    score >= SIGNAL_LIMITS.watchScore
  ) status = "WATCH";
  ...
  if (plan) plan.leverage = suggestedLeverage({ score, entry: plan.entry, stop: plan.stop, maxLeverage });
  ...
  return {
    ...
    timeframeBias: {
      "60": timeframeBias(states["60"]),
      "240": bias4h,
      D: bias1d,
    },
    reasons: buildReasons({ bias, states, spreadPct, volumeScore, availability }),
    spreadPct,
    availability,
    fresh,
    higherTimeframeConfirmed,
...
export function rankTurnover(tickers, symbols) {
  const rows = symbols
    .map((symbol) => ({ symbol, value: Number(tickers.get(symbol)?.turnover24h) || 0 }))
